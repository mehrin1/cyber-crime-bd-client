...
export function LawLibrary() {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/laws?${query.toString()}`, { signal: controller.signal })
      .then(async (response) => {
        ...
        if (!response.ok) {
          throw new Error(payload.message || "Unable to load the legal library.");
        }
        ...
        if ((loadError as { name?: string }).name !== "AbortError") {
          setError(
            loadError instanceof Error
              ? loadError.message
              : "Unable to load the legal library.",
          );
        }
      });
  ...
  const selectedLaw =
    laws?.find(
      (law) => law.slug === (selectedSlug || searchParams.get("law") || ""),
    ) ||
    laws?.[0] ||
    null;
  ...
  return (
    <>
      <section className="page-hero relative bg-[#092d2a]">
        ...
        <div className="page-container relative">
          ...
          <h1 className="mt-4 text-4xl font-black tracking-tight sm:text-5xl">
            Cyber law, evidence, and digital-rights frameworks.
          </h1>
          <p className="mt-5 max-w-2xl text-base leading-7 text-sky-100 sm:text-lg">
            Search current Bangladesh legal instruments and key international
            frameworks. Each record is source-linked and tagged so future
            cybercrime types can point to the relevant legal pathway.
          </p>
        </div>
      </section>
      ...
      <Card className="surface-card border-0 bg-background shadow-none">
        <CardContent className="grid gap-4 p-5 lg:grid-cols-[1.5fr_repeat(3,minmax(0,0.55fr))]">
          <label className="relative block">
            <span className="sr-only">Search legal library</span>
            <Search className="absolute top-3.5 left-3 size-4 text-muted-foreground" />
            <Input
              className="pl-9"
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder="Search law, topic, or problem, e.g. phishing, privacy, evidence"
            />
          </label>
          <select
      ...
      {!laws && !error ? (
        <p className="text-sm text-muted-foreground">Loading legal library...</p>
      ) : null}
      ...
      {laws ? (
        <section className="grid items-start gap-6 xl:grid-cols-[minmax(0,1.3fr)_minmax(330px,0.7fr)]">
          <div>
            <div className="mb-4 flex items-center justify-between gap-3">
              <p className="text-sm text-muted-foreground">
                {laws.length} legal record{laws.length === 1 ? "" : "s"}
              </p>
                  ...
                  >
                    <CardHeader className="gap-3 pb-3">
                      <div className="flex items-center justify-between gap-3">
                        <span
                          className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${statusClass(law.status)}`}
                        >
                          {statusLabels[law.status]}
                        </span>
                        <span className="text-xs font-medium text-muted-foreground">
                          {law.region === "BANGLADESH"
                            ? "Bangladesh"
                            : "International"}
                        </span>
                      </div>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {law.legalTopics.slice(0, 3).map((topic) => (
                          <span
                            key={topic}
          ...
          <aside className="xl:sticky xl:top-6">
            {selectedLaw ? (
              <Card className="overflow-hidden bg-background shadow-lg shadow-slate-950/5">
                <div className="bg-[linear-gradient(120deg,#ecfeff,#eff6ff)] p-6 dark:bg-muted">
                  <div className="flex items-start gap-3">
                    <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-sky-700 text-white">
                      <Landmark className="size-5" />
                    </span>
                    <div>
                      <p className="text-xs font-semibold tracking-[0.16em] text-sky-800 uppercase dark:text-sky-300">
                ...
                <CardContent className="space-y-6 p-6">
                  ...
                  <div className="grid gap-3 rounded-xl border bg-muted/30 p-4 text-sm">
                    <div>
                      ...
                      <p className="mt-1 font-medium">{selectedLaw.authority}</p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">
                        Published / status
                      ...
                      <p className="mt-1 font-medium">
                        ...
                        {statusLabels[selectedLaw.status]}
                      </p>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {selectedLaw.applicability.map((item) => (
                        <span
                          key={item}
                  ...
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {selectedLaw.legalTopics.map((topic) => (
                        <span
                          key={topic}
                          className="rounded-full bg-sky-50 px-2.5 py-1 text-xs text-sky-800 dark:bg-sky-950/40 dark:text-sky-200"
                        >
                          {topic}
                        </span>
                      ))}
                  ...
                  {selectedLaw.source ? (
                    ...
                    >
                      <ExternalLink />
                      Open official or primary source
                    </Button>
                  ) : null}
                  <p className="text-xs leading-5 text-muted-foreground">
                    General legal information only. Verify the current
                    legislation and obtain qualified legal advice for a specific
                    case.
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="p-6 text-sm text-muted-foreground">
                  No law matches the selected filters.
                </CardContent>
              </Card>
            )}
          </aside>
